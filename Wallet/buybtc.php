...
function Buy(){
    ...
    $LogsMessage = ($actualDate.'- User-'.$ID.' BTC Transaction + '.$lastValue.'New value: '.$newAmmount);
...
function SellCt(){
    ...
    if($criptoWallet - $sell<0){
        ...
        $actualDate = date('Y-m-d');
        $LogsMessage = ($actualDate.'- User-'.$ID.' BTC Transaction - '.floatval($sell).'New value: '.$newAmmount);
        $hash = password_hash($LogsMessage,PASSWORD_DEFAULT);
